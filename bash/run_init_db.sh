...
pathsql=`find $(pwd) -name demo.sql` && echo $pathsql  && docker cp echo $pathsql postgres-container:/var/lib/postgresql/data/

# Даем время на внесение изменений
sleep 5

#  Запускаем скрипт для заполнения БД
docker exec postgres-container psql -U test_sde -d demo -f //var/lib/postgresql/data/demo.sql
